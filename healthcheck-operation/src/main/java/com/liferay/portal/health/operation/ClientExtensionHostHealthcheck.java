	...
	public Collection<HealthcheckItem> check(long companyId) {
		...
		try {
			List<ClientExtensionEntry> clientExtensionEntries = ClientExtensionEntryLocalServiceUtil
					.getClientExtensionEntries(companyId, 0, 9999);
			...
			for (ClientExtensionEntry clientExtensionEntry : clientExtensionEntries) {
				UnicodeProperties typeSettings = UnicodePropertiesBuilder.create(true)
						.load(clientExtensionEntry.getTypeSettings()).build();
				String[] urls;
